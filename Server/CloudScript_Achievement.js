...
// Accomplish achievement issued by the client.
handlers.updateAchievement = function (args, context) {
    var achievementList_Remotelocal = args.achievementList;
    
    var updateUserAchievementDataResult = server.UpdateUserReadOnlyData({
        PlayFabId: currentPlayerId,
        Data: {
            UserAchievementList: achievementList_Remotelocal
        }
    });
    
    log.debug("Achievement updated for player " + currentPlayerId + " to " + achievementList_Remotelocal);
};

// // Accomplish achievement HighScore 10 issued by the client.
// handlers.checkScoreForAchievement = function (args){
//     var submittedScore = args.score;
    
//     var playerData = server.GetUserReadOnlyData({
//         PlayFabId: currentPlayerId,
//         Keys: ["UserAchievementList"]
//     }); 
...
//         if(submittedScore >= 10){
//             jsonAchievementList.achievement_strength_10 = true;
//             achivementList = JSON.stringify(jsonAchievementList);
//             var UpdateScoreAchievementResult_10 = server.UpdateUserReadOnlyData({
//                 PlayFabId: currentPlayerId,
//                 Data: {
//                     UserAchievementList: achivementList
//                 }
//             });
//             log.debug("Score_10 acquired");
//         }
//     } catch (e){
...
handlers.checkHighScoreForAchievement = function (){
    ...
    try{
        var achivementList = playerData.Data["UserAchievementList"].Value;
        var jsonAchievementList = JSON.parse(achivementList);
        if(jsonAchievementList.achievement_highscore_100 == false){
            jsonAchievementList.achievement_highscore_100 = true;
        }
        achivementList = JSON.stringify(jsonAchievementList);
        
        var UpdateScoreAchievementResult = server.UpdateUserReadOnlyData({
            PlayFabId: currentPlayerId,
            Data: {
                UserAchievementList: achivementList
            }
        });
        log.debug("HighScore_100 acquired");
    } catch (e){
        log.error("Error occurred: " + e.message);
    }
}

// Accomplish achievement LevelComplete issued by the client.
handlers.checkLevelCompleteForAchievement = function (args){
    var acomplishedLevel = args.LevelName;
    
    var playerData = server.GetUserReadOnlyData({
        PlayFabId: currentPlayerId,
        Keys: ["UserAchievementList"]
    });
    log.debug("LevelName acquired:" + acomplishedLevel);
    ...
    try{
        ...
        log.debug("LevelComplete acquired");
    } catch (e){
        log.error("Error occurred: " + e.message);
    }
}
...
handlers.initializePlayer = function (){
    var achievementList = JSON.stringify({"achievement_levelcompleted":false,"achievement_highscore_100":false});
    
    var InitAchievementForNewPlayer = server.UpdateUserReadOnlyData({
        PlayFabId: currentPlayerId,
        Data: {
    ...
    var AttributeHighScore = {
        StatisticName: "HighScore",
        Value: 0 //you can use a variable representing changed value
    };
    
    addingStats.push(AttributeHighScore);
    
    var InitStatisticsForNewPlayer = server.UpdatePlayerStatistics({
        PlayFabId: currentPlayerId,
